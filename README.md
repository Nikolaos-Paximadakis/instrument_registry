...
By default, the local cache lives at:

```text
instrument_registry/src/instrument_registry/instrument_registry.db
```

(gitignored, regenerated by `refresh_athex()`/`refresh_gleif()` — not
committed, same convention as `currens`'s `exchange_rates.db`.)
